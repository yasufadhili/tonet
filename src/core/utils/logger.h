 ...
 * Created Date: Monday 11th November, 2024 07:18:21 EAT
 * Author: Yasu Fadhili
 * 
...
namespace utils {
  ...
  public:
    ...
    explicit Logger(const std::string &file_name)
      ...
      logger_thread_ = createAndStartThread(-1, "utils/Logger " + file_name_, [this]() { flushQueue(); });
      ASSERT(logger_thread_ != nullptr, "Failed to start Logger thread.");
    }

    ~Logger() {
      std::string time_str;
      std::cerr << utils::getCurrentTimeStr(&time_str) << " Flushing and closing Logger for " << file_name_ << std::endl;
      ...
      file_.close();
      std::cerr << utils::getCurrentTimeStr(&time_str) << " Logger for " << file_name_ << " exiting." << std::endl;
    }
