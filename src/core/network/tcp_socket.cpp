 ...
 * Created Date: Monday 11th November, 2024, 21:27:41 EAT
 * Author: Yasu Fadhili
 * 
  ...
  auto TCPSocket::sendAndRecv() noexcept -> bool {
    ...
    if (read_size > 0) {
      ...
      if (cmsg->cmsg_level == SOL_SOCKET &&
        ...
        kernel_time = time_kernel.tv_sec * utils::NANOS_TO_SECS + time_kernel.tv_usec * utils::NANOS_TO_MICROS; // convert timestamp to nanoseconds.
      }

      const auto user_time = utils::getCurrentNanos();

      logger_.log("%:% %() % read socket:% len:% utime:% ktime:% diff:%\n", __FILE__, __LINE__, __FUNCTION__,
                  utils::getCurrentTimeStr(&time_str_), socket_fd_, next_rcv_valid_index_, user_time, kernel_time, (user_time - kernel_time));
    ...
    if (next_send_valid_index_ > 0) {
      ...
      logger_.log("%:% %() % send socket:% len:%\n", __FILE__, __LINE__, __FUNCTION__, utils::getCurrentTimeStr(&time_str_), socket_fd_, n);
    }
