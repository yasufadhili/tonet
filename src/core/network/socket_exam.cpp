...
int socket_examp() {
  ...
  utils::Logger logger_("socket_example.log");

  auto tcpServerRecvCallback = [&](net::TCPSocket *socket, utils::Nanos rx_time) noexcept {
  ...
  auto tcpClientRecvCallback = [&](net::TCPSocket *socket, utils::Nanos rx_time) noexcept {
  ...
  std::vector<net::TCPSocket *> clients(5);

  for (size_t i = 0; i < clients.size(); ++i) {
    clients[i] = new net::TCPSocket(logger_);
