 ...
 * Created Date: Monday 11th November, 2024, 21:08:33 EAT
 * Author: Yasu Fadhili
 * 
...
namespace net {
  ...
  [[nodiscard]] inline auto createSocket(utils::Logger &logger, const SocketCfg& socket_cfg) -> int {
    std::string time_str;

    const auto ip = socket_cfg.ip_.empty() ? getIfaceIP(socket_cfg.iface_) : socket_cfg.ip_;
    logger.log("%:% %() % cfg:%\n", __FILE__, __LINE__, __FUNCTION__,
               utils::getCurrentTimeStr(&time_str), socket_cfg.toString());
